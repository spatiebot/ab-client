...
export class PathFinding {
    ...
    public static findPath(config: IFindPathConfig): ICalculatedPath {
        ...
        if (!path || path.length * map.getScale() < PATH_LENGTH_THRESHOLD_FOR_HIGHRES) {
            ...
            if (path.length >= segmentLength) {
                const refinedPath = this.tryFindPath(highresAircraftMaps[aircraftType], path[0], path[segmentLength - 1]);
                if (refinedPath) {
    ...
    private static tryFindPath(map: IShipAwareMapRepresentation, pos1: IPos, pos2: IPos) {
        try {
            return map.findPath(pos1, pos2);
        } catch (error) {
            // tslint:disable-next-line: no-console
